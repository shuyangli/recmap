...
export const actionPanelReducer = TypedReducer.builder<ActionPanelState>()
  .withHandler(ToggleDetailPanel.TYPE, (state, { locationId }) => setWith(state, {
    locationId,
  ...
  .withHandler(ToggleEditPanel.TYPE, (state, { locationId }) => setWith(state, {
    locationId,
  ...
  .withHandler(ClosePanel.TYPE, (state) => setWith(state, { isVisible: false }))
  .build();
